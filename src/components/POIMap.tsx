...
const POIMap = ({ userId }: { userId?: number }) => {
  ...
  const { loading: userFavoritesLoading, error: userFavoritesError, data: userFavoritesData } = useQuery(
    GET_USER_FAVORITES,
    {
      variables: { userId },
      skip: !userId,
  ...
  useEffect(() => {
    if (userFavoritesData?.getUserFavorites && userId !== undefined) {
      ...
      userFavoritesData.getUserFavorites.forEach((favorite: { id: number; pointOfInterest: { id: number } }) => {
    ...
    function handleAddFavorite(poiId: number, favoriteId: number) {
      setFavorites((prevFavorites) => ({ ...prevFavorites, [poiId]: favoriteId }));
    }
